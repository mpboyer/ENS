...
\begin{frame}
    ...
    \visible<2->{On dit de plus qu'elle est à densité lorsqu'il existe une fonction $\mu_{p}$ telle que :
        ...
        \]
        où $\lambda$ correspond \textit{intuitivement} à la taille de l'ensemble $A$ puisqu'en particulier $\lambda(\left[a, b\right]) = b - a$.}
\end{frame}
...
\begin{frame}
    ...
    \only<1>{\begin{definition}
            On définit la Catégorie de Lusternick-Schnirelmann d'un espace $X$ noté $\text{cat}(X)$ comme le nombre minimal (moins 1) d'ensembles ouverts contractables en un point suffisant à le recouvrir.
        \end{definition}}
    \only<2>{
...
\begin{frame}
    \frametitle{Théorème de Partage de Nouilles}
    \begin{theorem}[De Partage de Nouilles]
            Pour toutes fonctions de densités $f_{i}$ et permutations $\sigma \in \mathfrak{S}_{n}$, on considère le système $(\star)$ suivant :
            \begin{equation*}\tag{$\star$}
                \int_{0}^{x_{1}}f_{\sigma(1)}(x) \mathrm{d}x = \int_{x_{1}}^{x_{2}}f_{\sigma(2)}(x)\mathrm{d}x = \ldots = \int_{x_{n}}^{1}f_{\sigma(n)}(x) \mathrm{d}x
            \end{equation*}
