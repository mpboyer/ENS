...
We defined $n$ as the number of elements, i.e. of \texttt{make_set} and $m$ the total number of operations.
...
\begin{proposition}
    Suppose we convert a sequence of $m^{'}$ operations \texttt{make_set, union_set, find_set} into $m$ operations \texttt{make_set, link, find_set}. If we can execute the latter in $\O(m \cdot \alpha(n))$, we can execute the former in $\O(m^{'} \cdot \alpha(n))$.
\end{proposition}

\begin{proof}
    $m^{'} \leq m \leq 3m^{'}$
\end{proof}
We will thus assume we have only \texttt{make_set, link, find_set} operations.
