...
We denote  by $i \uparrow k$, the event $x_{i}$ if a proper ancestor of $x_{k}$ : $\texttt{depth}(x_{k}) = \sum_{i = 1}^{i = n} \left[i \uparrow k\right]$
...
\begin{corollary}
    ...
    Then :
    \[
        \mathbb{E}\left[\texttt{depth}(x_{k})\right] = \sum_{i = 1}^{i = n}\mathbb{P}\left[i \uparrow k\right] = \sum_{i = 1}^{k - 1}\frac{1}{k - i + 1} + \sum_{k+1}^{n}\frac{1}{i - k + 1} = H_{k} - 1 + H_{n - k + 1} - 1 < 2 \ln n - 2
    \]
...
\begin{definition}[Recursive van Emde Boas Trees]
    \begin{itemize}
        \item \texttt{T.summary} is a vEB-tree of size $\sqrt{u}$ containing all $i$ such that the $i$-th chunk is not empty.
        \item For each $1 \leq i \leq \sqrt{u}$, \texttt{T.chunk[$i$]} is a vEB-tree of size $\sqrt{u}$ containing $x \texttt{mod} \sqrt{u}$ for each $x$ in the $i$-th tree.
        \item \texttt{T.min} is the smallest element in $T$, not stored recursively.
    \end{itemize}
    We represent each integer $x = \langle c, i \rangle$ where
    \begin{itemize}
        \item $c$ is the chunk coordinates : $c = x // \sqrt{u}$
        \item $i$ is the position of $x$ in the chunk : $i = x \texttt{mod} \sqrt{u}$
    \end{itemize}
\end{definition}
...
\begin{proposition}[Successor Operation]
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}
            ...
            \EndIf
            \If {$i < T.chunk[c].max$}
            \State \Return $\langle c, \texttt{Successor}\left(T.chunk[c], i\right) \rangle$
            \Else
            \State $c^{'} = \texttt{Successor}(T.summary, c)$
...
\begin{proposition}[Insertion]
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}
            ...
            \EndIf
            \If {$x < T.min$}
            \State $\texttt{swap}(x, T.min)$
            \EndIf
            \If {$x > T.max$}
            \State $T.max = x$
            \EndIf
            \If {$T.chunk[c].min = None$}
            \State $\texttt{Insert}(T.summary, c)$
            \EndIf
            \State $\texttt{Insert}(T.chunk[c], i)$
            \EndFunction
        \end{algorithmic}
    \end{algorithm}
\end{proposition}
...
\begin{proposition}[Deletion]
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}
            ...
            \EndIf
            \State $\texttt{Delete}(T.chunk[c], i)$
            \If {$T.chunk[c].min = None$}
            \State $\texttt{Delete}(T.summary, c)$
            \EndIf
...
\begin{proof}
    \begin{itemize}
        ...
        \item Time : $\texttt{Time}(B[k]) \leq \abs{B[k] - B[k - 1]}$, indeed, if $B[k] = B^{j}[k - 1] + 1$, we use $j$ steps.
              Observe that :
              \[
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \Input $\ \ T, P, n, m$
        \EndInput
        \State $B \gets \texttt{BorderArray}(P)$
        \State $L \gets []$
        \State $i \gets 1, \ j\gets 1$
        \While {$i \leq n - m + j$}
        \State $(i, j) \gets \texttt{match}(i, j, m)$
        \If {$j = m + 1$}
        \State $L \gets (i - m)::L$
        \Else
        \If {$j = 1$}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \While {$i \leq n$}
        \If {$\exists e = (curr \_node, u)$ labeled with $T[i]$}
        \State $curr\_node \gets u$
        \If {$curr\_node$ corresponds to a pattern}
        \State $L \gets i :: L$
        \EndIf
        \State $i \gets i + 1$
        \Else
...
We modify the algorithm : at each step it follows the path from $curr \_node$ and outputs the patterns in the output link path.
...
We can generalise this to any $m$ strings in time $\O(n)$ where $n = \sum_{k = 1}^{m} \abs{T_{k}}$. We will use the following fact : we can preprocess a tree of size $\O(n)$ in time $\O(n)$ to support lowest common ancestor ($LCA$) queries in constant time. (see ) $LCA(u, v)$ must return the lowest node that is an ancestor of both $u$ and $v$.
...
\begin{definition}
    ...
    \begin{itemize}
        \item \texttt{make\_set($x$)} : create a new set containing one element, $x$
        \item \texttt{union\_set($x, y$)} : union the sets containing $x$ and $y$
        \item \texttt{find\_set($x$)} : return a pointer to the representative of the set containing $x$
    \end{itemize}
...
\begin{proposition}
    In this situation : \texttt{make\_set} and \texttt{find\_set} are done in $\O(1)$.
\end{proposition}

For \texttt{union\_set}, it requires more work.\\
...
\[
    \texttt{make\_set}(x_{1}), \texttt{make\_set}(x_{2}), \ldots, \texttt{make\_set}(x_{n});\\
    \texttt{union\_set}(x_{2}, x_{1}), \texttt{union\_set}(x_{3}, x_{2}),\ldots, \texttt{union\_set}(x_{n}, x_{n-1})
\]
...
Here, each set is a treee whose root is the representative. Each node stores a pointer to its parent. \texttt{make\_set}($x$) creates a tree containing only one node, \texttt{find\_set}($x$) follows the path from $x$ to the root and \texttt{union\_set}($x, y$) makes the root of the tree of $y$ become a child of the root of the tree of $x$.\\
We will use two strategies :
\begin{itemize}
    \item Path Compression : During \texttt{find\_set}($x$), we make each node on the path to the root be a child of the root.
    \item Union by Rank : During \texttt{union\_set}, the root of the tree with smaller rank becomes a child of the root of the tree with larger rank.
\end{itemize}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \Function \texttt{union\_set}
        \Input \ \ $x, y$
        \EndInput
        \State \texttt{link(find\_set($x$), find\_set($y$))}
        \EndFunction

        \Function \texttt{find\_set}
        \Input \ \ $x$
        \EndInput
        \If {$x \neq x.p$}
        \State $x.p \gets find\_set(x.p)$
        \EndIf
        \Return $x.p$
        \EndFunction
...
We defined $n$ as the number of elements, i.e. of \texttt{make\_set} and $m$ the total number of operations.

\begin{theorem}[Tarjan] % FUCK TARJAN
    Using both Union by Rank and Path Compression, the time complexity for any sequence of Find and Union is $\O(m + n\alpha(n))$ where $\alpha(4)$ is the inverse of the Ackermann function. In any conceivable application $\alpha(n) \leq 4$. This time complexity is tight.
\end{theorem}

\subsection{A simpler Bound}
We will first start with a simpler analysis showing $\O(m \log^{\star} n)$.\\

\begin{proposition}
    Suppose we convert a sequence of $m^{'}$ operations \texttt{make\_set, union\_set, find\_set} into $m$ operations \texttt{make\_set, link, find\_set}. If we can execute the latter in $\O(m \cdot \alpha(n))$, we can execute the former in $\O(m^{'} \cdot \alpha(n))$.
\end{proposition}

\begin{proof}
    $m^{'} \leq m \leq 3m^{'}$
\end{proof}
We will thus assume we have only \texttt{make\_set, link, find\_set} operations.

\begin{proposition}
    \begin{itemize}
        \item The rank of a node can only increase with time. Rank of non-root nodes are frozen forever.
        \item For every node $x$, $x.rank \leq x.p.rank$ and the inequality is strict unless $x$ is a root node.
        \item The ranks of nodes in any path strictly increase
        \item The rank of any node is bounded by $n - 1$.
    \end{itemize}
\end{proposition}

\begin{proposition}[Rank Lemma]
    At any moment, for every $r \in \N$, there are at most $\frac{n}{2^{r}}$.
\end{proposition}

\begin{proof}
    By induction, the number of nodes in a tree with a root of rank $r$ is at least $2^{r}$ :
    \begin{itemize}
        \item Base Case : $r = 0$ is true
        \item Induction Step : After linking two trees with roots of rank $r_{1}, r_{2}$ :
              \begin{itemize}
                  \item If $r_{1} < r_{2}$ the claim is true by induction for $r_{2}$.
                  \item If $r = r_{1} = r_{2}$, then the root of the resulting tree has rank $r + 1$ and the tree has at least $2^{r} + 2^{r} = 2^{r + 1}$ nodes.
              \end{itemize}
    \end{itemize}
...
\subsection{Buckets}
\begin{definition}
    We define buckets so they form a partition of $\N$ with : $B_{0} = \left[0, 0\right], B_{1} = \left[1, 1\right]$, $B_{2} = \left[2, 2^{2} - 1\right], B_{3} = \left[2^{2}, 2^{(2^{2})} - 1\right]$ and so on. They are not implemented, bu only used for the analysis. The total number of buckets is at most $\log^{\star} n$.
\end{definition}

\begin{proposition}\label{bucketlemma}
    Put a node with rank $r$ into the bucket $B$ containing $r$. The number of nodes in a bucket $B = \left[b, 2^{b} - 1\right]$ is at most :
    \[
        \sum_{r \in B} \frac{n}{2^{r}} \leq \frac{2n}{2^{b}}
    \]
\end{proposition}
...
\begin{definition}
    ...
    \begin{itemize}
        \item It is a root
        \item It is a child of a root
        \item It is in a smaller bucket than its parent
    \end{itemize}
    It is bad otherwise.
\end{definition}
...
\subsection{Clique by Degree}
We define :
\begin{definition}
    Given a graph $G$ with maximum degree $\Delta$ and an integer $k$, does $G$ have a clique of size at least $k$ ?
\end{definition}

This is FPT since looking at the neighbourhoods of each vertex gives us $\O(2^{\Delta}n)$ time complexity. There might be no FPT solution for \textsc{Clique} parametrized by $k$.

\section{Algorithmic Techniques}
\subsection{Branching Method}

\subsubsection{$k$-\textsc{Vertex Cover}.}

\begin{definition}
    The main idea of the branching method is to reduce the problem to solving a bounded number of problems with parameter $k' < k$. Since the parameter decreases in every recursive call, the depth of the search tree is at most $k$. The size of the search tree branching into $c$ directions in $c^{k}$.\\
    We denote $\O(\alpha^{k}n^{\O(1)})$ by $\O^{\star}(\alpha^{k})$
\end{definition}
...
\begin{proposition}[Reduction Rules for FVS]
    \begin{enumerate}
...
\begin{proof}
    \begin{itemize}
        \item If the problem has a kernel, we reduce the size of the instance in poly-time and bruteforce on it.
        \item If the problem is solved in $f(k)\cdot \abs{x}^{c}$.\\
        If $\abs{x} \leq f(k)$ then we have our kernel. Else, if $\abs{x} \geq f(k)$ then we can solve the problem in time $f(k) \cdot \abs{x}^{c} \leq \abs{x}^{c + 1}$
    \end{itemize}
\end{proof}

The real issue is to know which problems have polynomial kernels. 

\subsubsection{\textsc{Vertex Cover}}
We define two reduction rules for $k$-\textsc{Vertex Cover} : 
\begin{proposition}
    \begin{itemize}
        \item If $v$ has degree $0$, then reduce to $(G - v, k)$
        \item If $v$ has degree $k + 1$, then reduce to $(G - v, k - 1)$.
    \end{itemize}
\end{proposition}
...
\begin{proposition}[Kernelization for \textsc{Vertex Cover}]
    \begin{itemize}
...
\begin{itemize}
    \item Assign color from $\left[k\right]$ to the vertices uniformly and independently at random
    \item Output YES if there is a path colored $1 - 2 - \cdots - k$ else NO
\end{itemize}
\begin{proposition}
    If $G$ has a $k$-path, the probability that this $k$-path is colored $1 - 2 - \cdots - k$ is $1/k^{k}$. So if $G$ is a YES instance the algo outputs YES with probability at least $1/(k^k)$, and if it is a NO instance, the algorithm outputs NO. Then, after $k$ repetitions, error probability is at most $1/e$. 
\end{proposition}
...
\begin{proposition}
    The final complexity of this algorithm is $\O(c\cdot k^{k} \cdot (n + m))$ and it has probability of success $1/e^{c}$.
\end{proposition}
...
We can solve this problem using Dynamic Programming : we solve for each vertex $v$ and set of color $C \subseteq \onen{k}$, the value of $D(v, C)$ that is YES is there is path ending at $v$ using each colour of $C$. \\
If we denote by $\chi : V \rightarrow [k]$ the random coloring, $D(v, C)$ is YES if and only if $\chi(v) \in C$ and there is an edge $uv$ for which $D(u, C\setminus \chi(v))$ is YES. We thus can solve this in time $2^{k}\times \abs{E}$.

Finally we get the following algorithm : 
\begin{proposition}
    Repeat $e^{k}$ times : 
    \begin{enumerate}
        \item Sample a coloring $c$
        \item Check if $G$ contains a colorful $k$-path in time $\O(2^{k}\cdot \abs{E})$ and return YES if it does. 
    \end{enumerate}
...
An approximation scheme is an algorithm scheme is an algorithm that outputs a $(1 + \epsilon)$-approximation where $\epsilon$ is part of the input.
\begin{definition}
    An algorithm is a Polynomial Time Approximation Scheme if it satifies
    \begin{itemize}
        \item Input : An instance $P$ and $\epsilon > 0$
        \item Output : A $(1+\epsilon)$-approximation
        \item Polynomial in $n$ for every fixed $\epsilon$
    \end{itemize}
    We say it is a Fully Polynomial Time Approximation Scheme if it is polynomial in $n$ and $1/\epsilon$.
\end{definition}

\subsection{\textsc{Min Vertex-Cover}}
\begin{algorithm}
    \caption{Naïve Algorithm}
    \begin{algorithmic}
        \State For each vertex $v$
        \If {$d(v) \geq 1$}
            \State Put $v$ in the solution
            \State Delete $v$
        \EndIf
    \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \State Choose Vertex with Maximum Degree
        \State Delete It
        \EndWhile
    \end{algorithmic}
\end{algorithm}
...
Assume there is a certain quantity $Q$ such that $Q \leq OPT$. If we can prove that $ALG \leq c \times Q$ then $ALG$ is a $c$-approximation. 
Here, a Vertex Cover is at least as large as a maximal matching. We get : 

\begin{algorithm}
    \caption{Based on Matching}
    \begin{algorithmic}
        \State $M \gets$ Maximal Matching
        \While {there are edges}
        \State Add $u$ and $v$ to the solution
        \State Delete $u$ and $v$
        \EndWhile
    \end{algorithmic}
\end{algorithm}
...
\subsection{\textsc{Set-Covering}}
\begin{definition}
    Given a hypergraph $\left(V, \mathcal{F}, c\right)$ where $V$ is finite a set of elements, $\mathcal{F}$ is a set of subsets of $V$ and $c$ is a cost function, output a cost minimum subset of $\mathcal{F}$ that covers all elements of $V$. 
\end{definition}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \While {$U \neq \emptyset$}
            \State Select $S \in \mathcal{F}$ that maximizes $\abs{S \cap U}$
            \State $\mathcal{C} \gets \mathcal{C} \cup \{S\}$
            \State $U \gets U \setminus S$
        \EndWhile
    \end{algorithmic}
\end{algorithm}
Yet, if there is a set in $\mathcal{F}$ of weight $10^{10}$ that is covered by two sets of weight $1$, the algorithm will choose the first option. So, we modify the greedy criteria to act on the average cost of newly covered vertices : $c(S) /\abs{S \cap U}$
\begin{algorithm}
    \caption{\textsc{Greedy Set-Cover}}
    \begin{algorithmic}
        \State $U \gets V$
        \State $\mathcal{C} \gets \emptyset$
        \While {$U \neq \emptyset$}
            \State Select $S \in \mathcal{F}$ that minimizes $\frac{c(S)}{\abs{S \cap U}}$
            \State $\mathcal{C} \gets \mathcal{C} \cup \{S\}$
            \State $U \gets U \setminus S$
        \EndWhile
    \end{algorithmic}
\end{algorithm}
\begin{theorem}
    \textsc{Greedy-Set-Cover} is a $\log(n)$-approximation.
\end{theorem}
\begin{proof}
    Define $price(v) = c(S)/\abs{S \cap U}$ where $S$ is the hyperedge that covers $v$ for the first time. \\
    The total cost of a solution is $\sum_{v \in V} price(v)$. Number the elements of $V$ by order of choose, with $v_{n}$ being the first chose one. We have : 
    \begin{lemma}
        For each $k \in \onen{n}, price(v_{k}) \leq OPT/k$
    \end{lemma}
    \begin{proof}
        Assume the algorithm is going to choose a set $S$ that is going to cover $v_{k}$ for the first time. Let $U$ be the set of uncovered elements at this moment. So $\{v_{k}, \ldots, v_{1}\} = U$.
        Since an optimal solution covers $U$ with at most $OPT$, there must be a set $S$ with cost-effectiveness at most $OPT/\abs{U}$. Hence, by the greedy price criteria
        \[
            price(v_{k}) \leq \frac{OPT}{\abs{U}} \leq \frac{OPT}{k}
        \]
    \end{proof}
    Hence, the total cost is : 
    \[
        \sum_{k = 1}^{n}price(v_{k}) \leq OPT \sum_{k = 1}^{n} \frac{1}{k} \leq OPT\cdot (\log n + 1)
    \]

    
\end{proof}
\begin{theorem}
    \textsc{Greedy-Set-Cover} does not have an approximation ration of $\left(1 - o(1)\right)\ln n$. 
\end{theorem}

\subsection{Layering and \textsc{Weighted Vertex-Cover}}
\begin{definition}
    Given a graph and a weight function, find a minimum weight vertex cover.
\end{definition}

\begin{definition}
    A degree weighted function of a graph $G$ is a function $\omega : V \rightarrow \N$ such that 
    \[
...
\begin{lemma}
    Let $G$ be a graph with weight function $\omega$. If $\omega$ is a degree weighted function, then $\omega(V) \leq 2\cdot OPT$
\end{lemma}
\begin{proof}
    We have : 
    \[
        \omega(V) =  \sum_{v \in V} \omega(v) \leq \sum_{v \in V}c\deg(v) \leq 2c\abs{E}
    \]
    If $S^{\star}$ is an optimal solution, 
    \[
        OPT = \omega(S^{\star}) = \sum_{v \in S^{\star}} \omega(v) = c \cdot \sum_{v \in S^{\star}} \deg v
    \]
    Since $S^{\star}$ covers all edges we have \[\sum_{v \in S^{\star}} \deg v \geq \abs{E}\]
\end{proof}
...
\begin{proposition}
    ...
    \begin{center}
        \begin{tabular}{p{.15\textwidth}p{.25\textwidth}p{.25\textwidth}c}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \For{each set of $n$ linearly independent constraints}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \While {There is a vertex of better objective value}
