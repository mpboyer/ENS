...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \EndInput
        \If {$[m, M] \cap [x_{1}, x_{2}] = \emptyset$}
            \State \Return $0$
        \EndIf

        \If {$[m, M] \subseteq [x_{1}, x_{2}]$}
...
\begin{proof}
    \begin{itemize}
        ...
        \item Everytime we go down in depth in the tree, we update the intervals to check in the children of the node considered. When arriving at a node sufficiently small, the answer is given, thus, this algorithm has complexity in $\O(\log_{2}(n))$
    \end{itemize}
\end{proof}
