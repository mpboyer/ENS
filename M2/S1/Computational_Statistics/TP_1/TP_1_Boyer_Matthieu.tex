...
On trouve donc:
\begin{equation*}
	\boxed{R(\that_{2}) = \V(\that_{2}) + \B(\that_{2}) = \frac{2\theta^{2}}{(n + 1)(n + 2)}}
\end{equation*}
...
\fbox{Pour cette mesure de risque, il vaut donc mieux utiliser l'estimateur $\that_{2} = \max X_{i}$} puisqu'il a un risque significativement plus petit (quand le nombre d'échantillon grandit).
...
\end{equation*}
\boxed{\text{Ainsi, on vérifie bien que $X \bot Y$ et que les deux variables suivent des lois normales de paramètres $0$, $1$.}}
...
Par la question $1$, \fbox{$X$ et $Y$ sont bien des lois normales de paramètres $0$, $1$ indépendantes.}

\subsection{Question 3}
\subsubsection{a)}
À chaque itération, $V_{1}$ et $V_{2}$ suivent une loi uniforme sur $[-1, 1]$ (par translation et dilatation des $U_{i}$).
La boucle continuant tant que $V_{1}^{2} + V_{2}^{2} > 1$, c'est à dire tant que le point est hors du disque unité, à la fin de la boucle, \boxed{\text{$(V_{1}, V_{2})$ est uniformément distribué sur le disque unité.}}
...
Le nombre d'itérations suit donc une loi géométrique avec probabilité de succès $p = \frac{\pi}{4}$, \fbox{le nombre d'étapes est donc $\frac{4}{\pi}$.}
...
\end{equation*}
\fbox{On vérifie bien que $T_{1}$ et $V$ sont indépendantes} ($T_{1}$ ne dépendant que d'une variable aléatoire indépendante de $V$) et que \fbox{$T_{1}$ a la même distribution que $\cos(\Theta) \sim \mU([0, 2\pi])$.}
...
\fbox{$(X, Y)$ sont des variables aléatoires qui suivent une loi gaussienne de paramètres $0$, $1$.}
