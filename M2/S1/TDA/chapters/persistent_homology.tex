\section{Persistence Topologique}
...
\begin{thm}\label{thm:persistence-stability}
	Pour toutes fonctions \emph{modérées} $f, g: \mathbb{X}\to \R$, $d_{B}(D_{f}, D_{g}) \leq \ninf{f - g}$.
\end{thm}
...
\begin{thm}
	Si $f, g : X \to \R$ sont \emph{modérées}, on a:
	\begin{equation*}
...
\subsubsection{Modules de Persistence}
...
\begin{thm}\label{thm:tame-distance-function}
	Si $\mathbb{X}$ et $\mathbb{Y}$ sont des espaces métriques pré-compacts:
	\begin{equation*}
		d_{\infty}(\rips(\X), \rips(\Y)) \leq 2d_{GH}(\X, \Y)
	\end{equation*}
\end{thm}
...
\begin{definition}
	On dit qu'un module de persistence est dit \define{$q$-modéré} si pour tout $a < b$, $v_{a}^{b}$ est de rang fini.
\end{definition}
Si $\X$ est pré-compact métrique, alors $H(\rips(\X))$ et $H(\cech(\X))$ sont $q$-modérés.

Cette condition apporte de forts théorèmes:
\begin{thm}\label{thm:tame-defined-module}
	Les modules de persistence $q$-modérés ont des diagrammes de persistence bien définis.
\end{thm}
...
\begin{thm}\label{thm:tame-distance-module}
	Si $\U$ et $\V$ sont $q$-modérés et $\epsilon$-entrelacés pour un certain $\epsilon \geq 0$, alors:
	\begin{equation*}
		d_{\infty}(\diag(\U), \diag(\V)) \leq \epsilon
	\end{equation*}
\end{thm}

On va donc chercher à construire des filtrations qui induisent par leurs groupes d'homologie des modules de persistence $q$-modérés,
et qui sont $\epsilon$-entrelacés quand les espaces/fonctions considérées sont $O(\epsilon)$-proches.
...
\begin{proposition}
	Si $\X, \Y$ sont des espaces métriques, pour tout $\epsilon > 2\d_{GH}\left(\X, \Y\right)$, alors $H\left(\rips\left(\X\right)\right)$ et $H\left(\rips\left(\Y\right)\right)$ sont $\epsilon$-entrelacés.
\end{proposition}
\begin{proof}
...
\begin{proposition}
	Si $\X, \Y$ sont des espaces métriques, pour tout $\epsilon \geq 2\d_{GH}\left(\X, \Y\right)$, alors $H\left(\cech\left(\X\right)\right)$ et $H\left(\cech\left(\Y\right)\right)$ sont $\epsilon$-entrelacés.
\end{proposition}
La preuve est similaire à celle d'avant.

\begin{thm}
	Soit $\X$ un espace métrique compact. Les modules de persistence associés à l'homologie des complexes $\rips(\X)$ et $\cech(\X)$ sont $q$-modérés.
\end{thm}
\begin{proof}
