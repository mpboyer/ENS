...
\end{equation*}
\section{Modèles de Mixtures Gaussiennes}
\subsection{Mixtures Gaussiennes}
\begin{definition}
	Une \define{mixture gaussienne} à $K$ composante est définie par la densité:
	\begin{equation*}
		\model{x}{\pi, \theta} = \sum_{k = 1}^{K}\pi_{k}\mN\left(x; \mu_k, \Sigma_{k}\right),
	\end{equation*}
	où $\theta = \left(\mu_{k}, \Sigma_{k}\right)_{k}$ et $\pi = \transpose{\left(\pi_{1}, \ldots, \pi_{K}\right)} \in \Delta_{K}$ est le vecteur des poids.
\end{definition}

En modèle graphique:
\begin{gmodel}{\plate{y}{y}{$\forall i \leq n$}}
	& \pi\ar[dl] \\
	\gmnode{obs, alias=y}x_{i} & \theta\ar[l]
\end{gmodel}

\begin{proposition}
	La log-vraisemblance d'une mixture Gaussienne est donnée par:
	\begin{equation*}
		\begin{aligned}
			L_{x_{1}, \ldots, x_{n}}\left(\pi, \theta\right) = & \sum_{i = 1}\log\model{x_{i}}{\pi, \theta}                                                   \\
			=                                                  & \sum_{i=1}^{n}\log \left(\sum_{k=1} \pi_{k}\mN\left(x_{i}; \mu_{k}, \Sigma_{k}\right)\right)
		\end{aligned}
	\end{equation*}
\end{proposition}

Optimiser ceci n'est pas trivial, puisqu'on n'a pas d'expression analytique pour les estimateurs de $\pi$ et $\theta$.

En écrivant $Z_{i} \sim \M(1, \pi)$ et $X_{i}\mid Z_{ik} = 1 \sim \mathcal{N}(\mu_{k}, \Sigma_{k})$, on a le modèle suivant:
\begin{gmodel}{\plate{z}{x}{$\forall i \leq n$}}
	\gmnode{lat, alias=z}z_{i}\ar[d] & \pi\ar[l]\\
	\gmnode{obs, alias=x}x_{i} & \theta\ar[l]
\end{gmodel}
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\State{$S_{n} \gets \left(\frac{\transpose{X}X}{\sigma^{2}} + \alpha I_{p}\right)^{-1}$}
		\State{$m_{n} \gets (\transpose{X}X + \alpha\sigma^{2}I_{p})^{-1}\transpose{X}Y$}
		\EndProcedure
		\Statex
		\Procedure{M}{$S_{n}, m_{n}$}
		\State{$\alpha \gets p / \left(\Tr\left(S_{n}\right) + m_{n}\transpose{m_{n}}\right)$}
		\State{$\sigma^{2}\gets \frac{1}{n}\left\{\norm{Y - Xm_{n}}^{2} +  \Tr(\transpose{X}XS_{n})\right\}$}
		\EndProcedure
		\Statex
		\Statex
		\Procedure{EM}{}
		\State{Initialiser $\alpha, \sigma^{2}$}
		\While{$L(\beta)$ change}
		\State{$E(\alpha, \sigma^{2})$}
		\State{$M(S_{n}, m_{n})$}
		\EndWhile
