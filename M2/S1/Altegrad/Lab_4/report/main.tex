...
\section*{Question 4}
Considering edgeless graphs with different node numbers give us the result, as their number of shortest paths will always be zero for any length and they can't be isomorphic as they are different size.
\section*{Task 10}
...
From Figure 5, we identify the graph structures:

\textbf{Graph G:} Nodes labeled $\{2, 1, 4, 5, 3, 1\}$ with edges: (2,1), (2,4), (2,5), (2,3), (1,5), (4,1), (4,3), (5,3)

\textbf{Graph G':} Nodes labeled $\{2, 1, 4, 5, 3, 4\}$ with edges: (2,1), (2,4), (2,5), (1,5), (4,3), (5,4), (5,3)

\paragraph*{Step 1: Collect Neighbor Labels for Each Node}

\textbf{Graph G:}
\begin{itemize}
	\item Node with label 2: neighbors = $\{1, 4, 5, 3\}$ $\rightarrow$ signature: $(2, \{1, 3, 4, 5\})$
	\item Node with label 1 (top right): neighbors = $\{2, 4, 5\}$ $\rightarrow$ signature: $(1, \{2, 4, 5\})$
	\item Node with label 4: neighbors = $\{2, 1, 3\}$ $\rightarrow$ signature: $(4, \{1, 2, 3\})$
	\item Node with label 5: neighbors = $\{2, 1, 3\}$ $\rightarrow$ signature: $(5, \{1, 2, 3\})$
	\item Node with label 3: neighbors = $\{2, 4, 5, 1\}$ $\rightarrow$ signature: $(3, \{1, 2, 4, 5\})$
	\item Node with label 1 (bottom): neighbors = $\{4, 3\}$ $\rightarrow$ signature: $(1, \{3, 4\})$
\end{itemize}

\textbf{Graph G':}
\begin{itemize}
	\item Node with label 2: neighbors = $\{1, 4, 5\}$ $\rightarrow$ signature: $(2, \{1, 4, 5\})$
	\item Node with label 1: neighbors = $\{2, 5\}$ $\rightarrow$ signature: $(1, \{2, 5\})$
	\item Node with label 4 (left): neighbors = $\{2, 3\}$ $\rightarrow$ signature: $(4, \{2, 3\})$
	\item Node with label 5: neighbors = $\{2, 1, 4, 3\}$ $\rightarrow$ signature: $(5, \{1, 2, 3, 4\})$
	\item Node with label 3: neighbors = $\{4, 5, 4\}$ $\rightarrow$ signature: $(3, \{4, 4, 5\})$
	\item Node with label 4 (bottom right): neighbors = $\{5, 3\}$ $\rightarrow$ signature: $(4, \{3, 5\})$
\end{itemize}

\paragraph*{Step 2: Generate New Labels}
Concatenate each node's label with its sorted neighbor labels.

\textbf{Graph G:}
\begin{itemize}
	\item $(2, [1, 3, 4, 5])$ $\rightarrow$ ``2-1-3-4-5''
	\item $(1, [2, 4, 5])$ $\rightarrow$ ``1-2-4-5''
	\item $(4, [1, 2, 3])$ $\rightarrow$ ``4-1-2-3''
	\item $(5, [1, 2, 3])$ $\rightarrow$ ``5-1-2-3''
	\item $(3, [1, 2, 4, 5])$ $\rightarrow$ ``3-1-2-4-5''
	\item $(1, [3, 4])$ $\rightarrow$ ``1-3-4''
\end{itemize}

\textbf{Graph G':}
\begin{itemize}
	\item $(2, [1, 4, 5])$ $\rightarrow$ ``2-1-4-5''
	\item $(1, [2, 5])$ $\rightarrow$ ``1-2-5''
	\item $(4, [2, 3])$ $\rightarrow$ ``4-2-3''
	\item $(5, [1, 2, 3, 4])$ $\rightarrow$ ``5-1-2-3-4''
	\item $(3, [4, 4, 5])$ $\rightarrow$ ``3-4-4-5''
	\item $(4, [3, 5])$ $\rightarrow$ ``4-3-5''
\end{itemize}

\paragraph*{Step 3: Count Label Occurrences}

\textbf{Graph G label counts:} Each of the 6 labels appears exactly once.

\textbf{Graph G' label counts:} Each of the 6 labels appears exactly once.

\paragraph*{Step 4: Compute Kernel Value}
The kernel value is the inner product of the label histograms (counting matching labels).

\textbf{Common labels after 1 WL iteration:} None of the refined labels match between G and G'.

\textbf{Kernel value:} $k(G, G') = 0$

\paragraph*{Interpretation}
The kernel value of \textbf{0 indicates no structural similarity} between graphs $G$ and $G'$ after one WL iteration. Despite both graphs having 6 nodes with similar label distributions (two nodes labeled 1, two labeled 4, and one each of 2, 3, and 5), none of the nodes share the same local neighborhood structure after refinement. This means that the way nodes connect to their neighbors differs completely between the two graphs, revealing fundamentally different local topologies. The WL algorithm successfully distinguishes these two non-isomorphic graphs in just one iteration.
\section*{Task 14}
